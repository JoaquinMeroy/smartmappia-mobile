  ...
  return (
    <div className="inline-flex items-center border border-brand-border rounded-lg overflow-hidden">
      <button
        ...
        disabled={disabled}
        className="px-2 py-1.5 active:bg-brand-surface disabled:opacity-40"
...
export default function ShopCartPage() {
  ...
  if (loading) {
    return (
      <MobilePortalShell variant="detail" title="Your cart" onBack={backToShop}>
      ...
      <MobilePortalShell variant="detail" title="Your cart" onBack={backToShop}>
        <Card className="p-8 text-center">
          ...
          <p className="text-sm text-brand-grey mt-1">
            Browse the stores and add something.
          </p>
  ...
  return (
    <MobilePortalShell variant="detail" title="Your cart" onBack={backToStore}>
      {error && (
      ...
      <div className="space-y-3 mb-5">
        {items.map((line) => {
          ...
          return (
            <Card
              key={line.id}
              className={`p-3 ${blocked ? "border-red-200" : ""}`}
            >
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-brand-grey mt-0.5">
                    {[line.variantLabel, line.brand, line.unit]
                      .filter(Boolean)
                      .join(" · ")}
                  </p>
                  <div className="flex items-center justify-between mt-2 gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={busyId === line.id}
                        className="p-1.5 text-brand-grey active:text-red-600 disabled:opacity-40"
                        aria-label="Remove"
                      >
              ...
              {!line.isAvailable && (
                <p className="mt-2 text-xs font-bold text-red-700 flex items-center gap-1">
                  <AlertTriangle className="w-3.5 h-3.5" /> No longer available
              ...
              {line.isAvailable && line.availability === "low" && (
                <p
                  className={`mt-2 inline-block px-2 py-0.5 rounded-full text-[11px] font-bold border ${TONE_CLASSES[meta.tone]}`}
                >
                  {meta.label}
                </p>
              )}
        ...
        {cart.vatAmount != null && (
          <div className="flex justify-between">
            <span className="text-brand-grey">
              VAT ({Math.round((cart.vatRate || 0) * 100)}%)
            </span>
      ...
      {!cart.meetsMinimum && (
        <p className="mt-3 text-sm text-amber-700 font-medium text-center">
          Add {fmtSAR(cart.shortfall)} more to reach the {fmtSAR(cart.minOrder)}{" "}
          minimum.
        </p>
      )}
