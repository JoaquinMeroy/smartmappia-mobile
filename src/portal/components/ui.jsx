...
export function UserMenu() {
  ...
  const displayName =
    profile?.fullName || user.email?.split("@")[0] || "Account";
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="text-sm font-bold text-brand-dark max-w-[110px] truncate hidden md:block">
          {firstName}
        </span>
        <ChevronDown
          className={`w-3.5 h-3.5 text-brand-grey hidden md:block transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-60 bg-white border border-brand-border rounded-2xl shadow-xl overflow-hidden z-50">
          <div className="px-4 py-3 border-b border-brand-border">
            <p className="font-black text-brand-black text-sm truncate">
              {displayName}
            </p>
          ...
          <button
            ...
            className={
              itemClass + " border-t border-brand-border !text-red-600"
            }
          >
  ...
  return (
    <div className="min-h-screen bg-brand-muted">
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-brand-border">
        <div
          className={`${wide ? "max-w-7xl" : "max-w-5xl"} mx-auto px-4 md:px-6 h-16 flex items-center justify-between gap-3`}
        >
          ...
          <div className="text-center leading-tight min-w-0">
            ...
            {subtitle && (
              <div className="text-xs text-brand-grey truncate">{subtitle}</div>
            )}
          </div>
      ...
      <main
        className={`${wide ? "max-w-7xl" : "max-w-5xl"} mx-auto px-4 md:px-6 py-5 md:py-6`}
      >
        {children}
      </main>
    </div>
...
export function Card({ children, className = "" }) {
  return (
    <div
      className={`bg-white border border-brand-border rounded-2xl shadow-sm ${className}`}
    >
      {children}
    </div>
  );
