            ...
            <div className="flex flex-col items-center">
              ...
              >
                {done ? (
                  ...
                  <span
                    className={`w-2 h-2 rounded-full ${active ? "bg-brand-orange animate-pulse" : ""}`}
                  />
                )}
              </div>
              {i < FOOD_TRACK_STEPS.length - 1 && (
                <div
                  className={`w-0.5 flex-1 min-h-5 ${done ? "bg-brand-orange" : "bg-brand-border"}`}
                />
              )}
            </div>
            <div className="pb-5 min-w-0">
              <p
                className={`text-sm font-bold ${done || active ? "text-brand-dark" : "text-brand-grey/60"}`}
              >
                {step.label}
                {active && (
                  <span className="ml-2 text-xs font-medium text-brand-orange">
                    Now
                  </span>
                )}
              </p>
...
const FOOD_TRACK_PHASE = {
  pending: {
    eta: "Confirming",
    note: "Waiting for the restaurant to confirm your order",
  },
  ...
  rider_to_restaurant: {
    eta: null,
    note: "Your rider is heading to the restaurant",
  },
      ...
      <div className="p-5 bg-gradient-to-br from-brand-orange/10 to-white">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-3xl font-black text-brand-black leading-none tabular-nums">
              {eta}
            </p>
  ...
  return (
    <Card className="p-5 space-y-3">
      ...
      <Row
        label="Order number"
        value={
          <span className="font-mono font-bold text-brand-dark">{code}</span>
        }
      />
      {from && (
        <Row
          label="Order from"
          value={
            <span className="font-bold text-brand-dark inline-flex items-center gap-1.5">
              <Store className="w-3.5 h-3.5 text-brand-orange" /> {from}
            </span>
          }
        />
      )}
      {address && (
        <Row
          ...
          value={<span className="text-brand-dark text-right">{address}</span>}
        />
      )}
      {total != null && (
        <Row
...
function RiderCard({ rider, etaMinutes }) {
  ...
  return (
    <Card className="p-5">
      <p className="text-xs font-bold text-brand-grey uppercase tracking-wider mb-3">
        Your delivery rider
      </p>
      <div className="flex items-center gap-3">
        ...
        {etaMinutes != null && (
          <div className="text-right shrink-0">
            <p className="text-xl font-black text-brand-orange tabular-nums leading-none">
              {etaMinutes}
            </p>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        <a
          ...
          className={
            btnGhost +
            " !py-2.5" +
            (rider.mobile ? "" : " pointer-events-none opacity-50")
          }
        >
        ...
        <a
          href={
            whatsappLink(
              rider.whatsapp || rider.mobile,
              "Hi, I am tracking my Smart Mappia food order.",
            ) || undefined
          }
          target="_blank"
          rel="noreferrer"
          className={
            "inline-flex items-center justify-center gap-2 bg-green-600 active:bg-green-700 text-white font-bold py-2.5 px-5 rounded-xl text-sm transition-all" +
            (whatsappLink(rider.whatsapp || rider.mobile)
              ? ""
...
export default function FoodTrackPage() {
      ...
      status: (payload) => {
        if (payload && payload.riderLocation) {
          setRiderLive({
            ...
            etaMinutes: payload.riderEtaMinutes ?? null,
          });
        } else {
          load();
        }
  ...
  const activeIdx = order
    ? trackStepIndex(FOOD_TRACK_STEPS, order.status, deliveryStatus)
    : -1;
  const progress =
    activeIdx >= 0 ? activeIdx / (FOOD_TRACK_STEPS.length - 1) : 0;
  const phase = (order && FOOD_TRACK_PHASE[stepId]) || { eta: "", note: "" };
  const liveEta = riderLive?.etaMinutes ?? data?.rider?.etaMinutes ?? null;
  const pickedUp =
    deliveryStatus === "picked_up" || order?.status === "out_for_delivery";
  const dropoffPoint =
  ...
  const restaurantPoint =
    data?.merchant && data.merchant.lat != null
      ...
      : null;
  const routeTarget = pickedUp
    ? dropoffPoint
    : (restaurantPoint ?? dropoffPoint);
  const heroEta =
        ...
        {order && (
          <>
                ...
                <div className="p-4 rounded-xl bg-red-50 border border-red-200 flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    {order.payment_status === "paid" && (
                      <p className="text-red-600 mt-0.5">
                        Your payment of {fmtSAR(order.total)} will be refunded —
                        usually within a few hours, and up to one working day.
                      </p>
                    )}
              ...
              <RoutedRideMap
                ...
                markers={[
                  ...
                  data.merchant && data.merchant.lat != null
                    ? {
                        type: "pickup",
                        lat: data.merchant.lat,
                        lng: data.merchant.lng,
            ...
            {data.rider && !failed && (
              <RiderCard
                rider={data.rider}
                etaMinutes={riderLive?.etaMinutes ?? data.rider.etaMinutes}
              />
            )}
              ...
              <Card className="p-5 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-brand-grey">
                    We're matching your order with the nearest available rider.
                  </p>
                </div>
              </Card>
            )}
            ...
            {!failed &&
              ...
              order.payment_status === "awaiting" && (
                <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-sm font-bold text-amber-800">
                  Cash on delivery: please have {fmtSAR(order.total)} ready for
                  your rider.
                </div>
              )}
            ...
            {order.status === "delivered" && (
              <Card className="p-5 text-center bg-gradient-to-br from-brand-orange/10 to-white border-brand-orange/30">
                ...
                <p className="font-black text-brand-black">
                  Thank you for ordering with Smart Mappia!
                </p>
                <p className="text-xs text-brand-grey mt-1 mb-4">
                  We hope you enjoy your meal. Your receipt is ready to
                  download.
                </p>
                <button
                  type="button"
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 bg-brand-orange active:bg-brand-orange/90 text-white font-black py-3 rounded-xl transition-all shadow-lg shadow-brand-orange/20 text-sm"
                >
            ...
            <Card className="p-5">
              ...
              <div className="mt-4 pt-3 border-t border-brand-border space-y-1.5 text-sm">
                      ...
                      <span className="font-bold tabular-nums">
                        {fmtSAR(
                          Number(order.delivery_fee) -
                            Number(order.delivery_surcharge),
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold tabular-nums">
                        {fmtSAR(order.delivery_surcharge)}
                      </span>
                    </div>
                  </>
                ) : (
                ...
                {Number(order.vat_amount) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-brand-grey">
                      {vatLabel(order.vat_rate)}
                    </span>
                    <span className="font-bold tabular-nums">
                      {fmtSAR(order.vat_amount)}
                    </span>
                  </div>
                )}
                ...
                {order.payment_method && (
                  <div className="flex justify-between pt-1.5">
                    ...
                    <span className="font-bold text-brand-dark">
                      {FOOD_PAYMENT_METHODS[order.payment_method] ||
                        order.payment_method}
                    </span>
                  </div>
                )}
