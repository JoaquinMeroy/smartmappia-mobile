...
import { downloadRideInvoice } from "../lib/invoice";
...
import { Card, Badge, Spinner, btnGhost } from "../components/ui";
...
export default function TrackPage() {
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      setData(d);
      doneRef.current =
        d.bookingStatus === "completed" || d.bookingStatus === "cancelled";
    } catch (err) {
    ...
    `booking-${code}`,
    {
      status: (payload) => {
        if (payload.driverLocation)
          setLive((l) => ({ ...l, driverLocation: payload.driverLocation }));
        if (payload.etaMinutes != null)
          setLive((l) => ({ ...l, etaMinutes: payload.etaMinutes }));
        if (payload.bookingStatus || payload.paymentStatus || payload.driver)
          refetch();
      },
    },
    realtimeEnabled,
  ...
  useEffect(() => {
    ...
    const pickup = resolveCoordsFromAddress(
      data.pickupAddress,
      data.pickupLat,
      data.pickupLng,
    );
    const dropoff = resolveCoordsFromAddress(
      data.dropoffAddress,
      data.dropoffLat,
      data.dropoffLng,
    );
    ...
    let from = null;
    let to = null;
    if (driverLoc && pickup && !tripStarted) {
      from = driverLoc;
      to = pickup;
  ...
  useEffect(() => {
    ...
    if (prev && (prev.lat !== cur.lat || prev.lng !== cur.lng)) {
      const dt =
        cur.at && prev.at
          ? (new Date(cur.at) - new Date(prev.at)) / 1000
          : null;
      setMovement({
        ...movementFrom(prev, cur, dt),
        heading: bearingDeg(prev, cur),
      });
    }
  ...
  useEffect(() => {
    ...
    const msgs = {
      ...
      completed: {
        t: "Trip completed — thank you for riding",
        icon: "success",
      },
    };
    ...
    return (
      ...
      >
        <Card className="p-6 text-center">
          ...
          <Link to="/book" className={btnGhost}>
            Make a new booking
          </Link>
        </Card>
      </MobilePortalShell>
  ...
  const driverLoc = live.driverLocation || data.driverLocation;
  const driverHeading = driverLoc?.heading ?? movement.heading;
  const driverSpeedKmh = driverLoc?.speedKmh ?? movement.speedKmh;
  const eta =
    live.etaMinutes ?? data.liveEtaMinutes ?? route?.durationMin ?? null;
  const isActive = ACTIVE.includes(data.driverRideStatus);
  const waiting =
    data.bookingStatus === "confirmed" ||
    data.bookingStatus === "payment_under_review";
  const tripStarted = data.driverRideStatus === "started";
  ...
  const mapLegend = [
    ...
    driverLoc && { svg: LEGEND_CAR_SVG, color: "#FF7E21", label: "Driver" },
  ].filter(Boolean);
  ...
  const line =
    route?.coords && route.coords.length >= 2 ? route.coords : straight;

  const canCancel = CANCELLABLE.includes(data.bookingStatus);
  const waLink =
    data.driver &&
    whatsappLink(data.driver.whatsapp, `Hi, this is about booking ${code}`);
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          {waiting && <Spinner />}
          <div>
            <div className="font-black text-lg text-brand-black leading-tight">
              {meta.label}
            </div>
            <div className="text-xs text-brand-grey">
              {waiting
                ? "Hang tight — matching you with a nearby driver…"
                : `Driver status: ${data.driverRideStatus || "—"}`}
            </div>
          </div>
        </div>
          ...
          {eta != null && line && (
            <div className="absolute top-4 left-4 z-[400] bg-white rounded-xl shadow-lg border border-brand-border px-4 py-2 flex items-center gap-2">
              ...
              <div>
                ...
                <div className="font-black text-brand-black leading-tight">
                  {eta} min
                </div>
                {isActive &&
                  driverLoc &&
                  (driverSpeedKmh != null &&
                  (movement.moving || driverSpeedKmh >= 3) ? (
                    <div className="text-[10px] font-bold text-green-600 leading-none mt-0.5">
                      Moving · ~{Math.round(driverSpeedKmh)} km/h
                    </div>
                  ) : (
                    <div className="text-[10px] font-bold text-brand-grey leading-none mt-0.5">
                      Stopped
                    </div>
                  ))}
              </div>
        ...
        <div className="space-y-4">
          {data.driver ? (
            <Card className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="font-black text-brand-black truncate">
              ...
              {waLink && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mt-3 w-full inline-flex items-center justify-center gap-2 bg-green-500 active:bg-green-600 text-white font-bold py-2.5 rounded-xl text-sm transition-colors"
                >
                  <MessageCircle className="w-4 h-4" /> Message driver on
                  WhatsApp
                </a>
              )}
            </Card>
            ...
            <Card className="p-4 text-center text-sm text-brand-grey">
              {waiting
                ? "No driver yet — waiting for one to accept your trip."
                : "—"}
            </Card>
          )}
          ...
          {data.paymentMethod === "cash" &&
            data.bookingStatus !== "completed" && (
              <Card className="p-4 flex items-center gap-3 bg-brand-warm border-brand-orange/30">
                <span className="w-10 h-10 rounded-xl bg-brand-orange text-white flex items-center justify-center shrink-0">
                  <Banknote className="w-5 h-5" />
                </span>
                <div className="min-w-0">
                  <p className="font-bold text-brand-dark text-sm">Cash trip</p>
                  <p className="text-xs text-brand-grey">
                    Pay SAR {Number(data.fareAmount).toFixed(2)} to your driver
                    in cash at pickup.
                  </p>
                </div>
              </Card>
          ...
          {data.bookingStatus === "completed" && (
            <Card className="p-5 text-center bg-gradient-to-br from-brand-orange/10 to-white border-brand-orange/30">
              ...
              <p className="font-black text-brand-black">
                Thank you for riding with Smart Mappia!
              </p>
              <p className="text-xs text-brand-grey mt-1 mb-4">
                We hope you had a great trip. Your invoice is ready to download.
              </p>
              <button
                type="button"
                onClick={() =>
                  downloadRideInvoice({
                    booking: {
                      booking_code: data.bookingCode || code,
                      trip_type: data.tripType,
                      pickup_address: data.pickupAddress,
                      pickup_street: data.pickupStreet,
                      pickup_building: data.pickupBuilding,
                      dropoff_address: data.dropoffAddress,
                      dropoff_street: data.dropoffStreet,
                      dropoff_building: data.dropoffBuilding,
                      ...
                      payment_method:
                        data.paymentMethod === "cash" ? "Cash" : "STC Pay",
                      passenger_mobile:
                        data.passengerMobile || data.passengerWhatsapp,
                    },
                    customerName:
                      profile?.fullName ||
                      user?.email ||
                      "Smart Mappia customer",
                    driverName: data.driver?.name,
                  })
                }
                className="w-full inline-flex items-center justify-center gap-2 bg-brand-orange active:bg-brand-orange/90 text-white font-black py-3 rounded-xl transition-all shadow-lg shadow-brand-orange/20 text-sm cursor-pointer"
              >
          ...
          {data.bookingStatus === "completed" && data.driver && session && (
            <RateDriver kind="ride" code={code} driverName={data.driver.name} />
          )}
          ...
          <Card className="p-4 space-y-3">
            <div className="flex gap-3">
              ...
              <div className="min-w-0">
                <div className="text-[10px] font-bold text-brand-grey uppercase">
                  Pickup
                </div>
                <div className="text-sm font-medium text-brand-dark">
                  {formatAddressDetail({
                    street: data.pickupStreet,
                    building: data.pickupBuilding,
          ...
          <EditableContact
            value={data.passengerMobile || data.passengerWhatsapp}
            editable={
              !!session &&
              !["completed", "cancelled"].includes(data.bookingStatus)
            }
            onSave={async (phone) => {
              await api.updateBookingContact(code, { passenger_mobile: phone });
              await refetch();
            }}
          />
          ...
          {canCancel && (
            <button
              onClick={onCancel}
              disabled={cancelling}
              className="w-full inline-flex items-center justify-center gap-2 bg-white active:bg-red-50 text-red-600 font-bold py-3 rounded-xl border border-red-200 transition-colors cursor-pointer disabled:opacity-50"
            >
              {cancelling ? (
                <Spinner />
              ) : (
          ...
          {(data.timeline || []).map((e, i) => (
            <li key={i} className="flex gap-3">
              <div className="flex flex-col items-center">
                <span
                  className={`w-3 h-3 rounded-full ${i === data.timeline.length - 1 ? "bg-brand-orange" : "bg-brand-border"}`}
                />
              ...
              <div className="pb-1 -mt-1">
                ...
                {e.message && (
                  <div className="text-xs text-brand-grey">{e.message}</div>
                )}
